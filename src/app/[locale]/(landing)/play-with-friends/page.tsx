...
const guide = problemGuides['play-with-friends'];

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  const { locale } = await params;
  const canonicalUrl = await getCanonicalUrl('/play-with-friends', locale);
...
export default async function PlayWithFriendsPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
