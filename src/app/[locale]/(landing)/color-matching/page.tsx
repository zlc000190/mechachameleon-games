...
const guide = problemGuides['color-matching'];

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  const { locale } = await params;
  const canonicalUrl = await getCanonicalUrl('/color-matching', locale);
...
export default async function ColorMatchingPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
